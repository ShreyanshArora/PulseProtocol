...
struct InputView: View {
    ...
    @State private var inputHistory: [(type: HapticType, correct: Bool)] = []
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 30) {
                ...
                HStack {
                    ...
                    HStack(spacing: 8) {
                        ForEach(0..<3, id: \.self) { index in
                            Circle()
                                .fill(index < controller.session.livesRemaining ? Color(hex: "FF6B6B") : Color.white.opacity(0.15))
                    ...
                    VStack(alignment: .trailing, spacing: 4) {
                        Text("ROUND \(controller.session.currentRound)")
                            .font(.system(size: 11, weight: .semibold, design: .monospaced))
                            .foregroundColor(.white.opacity(0.5))
                        Text("\(controller.session.roundScore)")
                            .font(.system(size: 32, weight: .bold, design: .rounded))
                            .foregroundColor(controller.session.roundScore >= 0 ? .green : .red)
                    }
                }
                ...
                // Perfect Streak
                if controller.session.perfectStreak > 0 {
                    HStack(spacing: 5) {
                        Image(systemName: "flame.fill")
                            .foregroundColor(.orange)
                        Text("\(controller.session.perfectStreak)x Perfect Streak!")
                ...
                VStack(spacing: 20) {
                    ...
                    // Scoring Guide
                    HStack(spacing: 20) {
                        ScoreLabel(icon: "star.fill", text: "Perfect: +10", color: .yellow)
                        ScoreLabel(icon: "checkmark.circle.fill", text: "Good: +5", color: .green)
            ...
            DragGesture(minimumDistance: 0)
                .onChanged { _ in
                    if !isPressed && controller.session.phase == .waitingForInput {
                        ...
                        // Determine type based on current duration for haptic feedback
                        let currentType = determineHapticType(duration: 0.1)
                        controller.onTapDown(type: currentType)
                ...
                .onEnded { _ in
                    if isPressed {
                        ...
                        // Determine the final type based on hold duration
                        let finalType = determineHapticType(duration: holdDuration)
                        
                        // Record the input with the final type
                        controller.onTapUp(type: finalType)
                        
                        // Add to history with animation
                        if let lastInput = controller.session.userInputs.last,
                           let sequence = controller.session.currentSequence {
                            let index = controller.session.userInputs.count - 1
                            if index < sequence.patterns.count {
                                let isCorrect = lastInput.type == sequence.patterns[index]
                                inputHistory.append((lastInput.type, isCorrect))
                            }
                        }
    ...
    // FIXED: Helper function to determine haptic type based on duration
...
// ScoreLabel is already defined in GameView.swift - no need to redeclare
...
struct PatternIndicator: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            // Expected timing
            Text(String(format: "%.2fs", expectedPattern.duration))
